...
parser.add_argument('--img_dir', type=str, default = 'example/361.png')
...
for i, img_file in enumerate(img_list):
    ...
    #如果输入是灰度图（ndim == 2），通过 np.expand_dims 添加通道维度。
    if img.ndim == 2:
        img = np.expand_dims(img,2)
        #使用 np.repeat 将单通道复制 3 次，生成伪 RGB 图像。
    ...
    if args.cuda and torch.cuda.is_available():
        img = img.cuda()
   #获取图像的原始高度和宽度，后续用于调整图像尺寸和恢复预测结果。
    ...
    with torch.no_grad():
        ...
        _, out = Model(img)  # ✅ 论文中的 forward time 就是这行的耗时
        end = time.time()
        if args.cuda and torch.cuda.is_available():
            torch.cuda.synchronize()
        ...
        # 打印运行时间（毫秒）
        print(f"=> Forward running time (1 frame, no flip): {(end - start) * 1000:.3f} ms")

        # 下面是原来的 flip 推理，不计入 time
