...
# __all__ = ['mish','Mish']
# WS，weight standardization权重参数标准化
...
# pre-activation based conv block，MyConv与正常卷积不同
...
# Deep Feature Fxtractor，本文选择的backbone
class deepFeatureExtractor_ResNext101(nn.Module):
    def __init__(self, args, lv6=False, in_ch=1856):
        ...
        self.lv6 = lv6
        self.conv1 = nn.Conv2d(in_ch, 64, kernel_size=1, padding=0, stride=1)
        self.conv2 = nn.Conv2d(in_ch, 256, kernel_size=1, padding=0, stride=1)
        self.conv3 = nn.Conv2d(in_ch, 512, kernel_size=1, padding=0, stride=1)
        self.conv4 = nn.Conv2d(in_ch, 1024, kernel_size=1, padding=0, stride=1)
        # self.conv1 = nn.Conv2d(in_ch=, out_ch=)
        if lv6 is True:
            self.layerList = ['relu', 'layer1', 'layer2', 'layer3', 'layer4']  # 对应后面的out-featlist
    ...
    def forward(self, x):
        ...
        for k, v in self.encoder._modules.items():
            ...
            if any(x in k for x in self.layerList):
                out_featList.append(feature)  # list 0
        feature1 = out_featList[0]
        feature2 = out_featList[1]
        feature3 = out_featList[2]
        feature4 = out_featList[3]

        """layer1"""
        feature1_down1 = F.interpolate(feature2, scale_factor=2)
        feature1_down2 = F.interpolate(feature3, scale_factor=4)
        feature1_down3 = F.interpolate(feature4, scale_factor=8)
        feature1_layer1 = torch.cat((feature1, feature1_down3, feature1_down2, feature1_down1), dim=1)
        output1 = self.conv1(feature1_layer1)
        """layer2"""
        feature2_down1 = F.interpolate(feature1, scale_factor=0.5)
        feature2_down2 = F.interpolate(feature3, scale_factor=2)
        feature2_down3 = F.interpolate(feature4, scale_factor=4)
        feature2_layer2 = torch.cat((feature2, feature2_down1, feature2_down2, feature2_down3), dim=1)
        output2 = self.conv2(feature2_layer2)
        """layer3"""
        feature3_down1 = F.interpolate(feature1, scale_factor=0.25)
        feature3_down2 = F.interpolate(feature2, scale_factor=0.5)
        feature3_down3 = F.interpolate(feature4, scale_factor=2)
        feature3_layer3 = torch.cat((feature3, feature3_down1, feature3_down2, feature3_down3), dim=1)
        output3 = self.conv3(feature3_layer3)
        """layer4"""
        feature4_down1 = F.interpolate(feature1, scale_factor=0.125)
        feature4_down2 = F.interpolate(feature2, scale_factor=0.25)
        feature4_down3 = F.interpolate(feature3, scale_factor=0.5)
        feature4_layer4 = torch.cat((feature4, feature4_down1, feature4_down2, feature4_down3), dim=1)
        output4 = self.conv4(feature4_layer4)

        # 将结果写入 out_featList
        out_featList[0] = output1
        out_featList[1] = output2
        out_featList[2] = output3
        out_featList[3] = output4

        return out_featList

    # 返回的是一个list，就是经过backbone后的不同层的结果
...
class deepFeatureExtractor_VGG19(nn.Module):
    def __init__(self, args, lv6=None):
...
class ConvGRUCell(nn.Module):
    def __init__(self, in_channels, hidden_channels, kernel_size=3):
        ...
        # 用于计算 z 门（更新门）
        self.conv_z = nn.Conv2d(in_channels + hidden_channels, hidden_channels, kernel_size=kernel_size,
                                padding=padding)
        # 用于计算 r 门（重置门）
        self.conv_r = nn.Conv2d(in_channels + hidden_channels, hidden_channels, kernel_size=kernel_size,
                                padding=padding)
        # 用于计算 q 候选隐藏状态
        self.conv_q = nn.Conv2d(in_channels + hidden_channels, hidden_channels, kernel_size=kernel_size,
                                padding=padding)

    def forward(self, x, h):
        """
        前向传播
        :param x: 输入特征图，形状为 [batch, in_channels, height, width]
        :param h: 隐藏状态，形状为 [batch, hidden_channels, height, width]
        :return: 更新后的隐藏状态 h
        """
        # 如果 h 是 None，则初始化为 0
        if h is None:
...
# Laplacian Decoder Network，decoder层
class Lap_decoder_lv5(nn.Module):
    def __init__(self, args, dimList):
        ...
        kSize = 3
        # 定义 ConvGRU 单元
        self.conv_gru1 = ConvGRUCell(in_channels=dimList[2], hidden_channels=dimList[3]//2)
        self.conv_gru2 = ConvGRUCell(in_channels=dimList[1], hidden_channels=dimList[3]//4)
        self.conv_gru3 = ConvGRUCell(in_channels=dimList[0], hidden_channels=dimList[3]//16)
        ...
        self.decoder1 = nn.Sequential(
            myConv(dimList[3], dimList[3] // 2, kSize, stride=1, padding=kSize // 2, bias=False, norm=norm,
                   act=act, num_groups=(dimList[3] // 2) // 16),
            myConv(dimList[3] // 2, dimList[3] // 4, kSize, stride=1, padding=kSize // 2, bias=False, norm=norm,
                   act=act, num_groups=(dimList[3] // 2) // 16),
            myConv(dimList[3] // 4, dimList[3] // 8, kSize, stride=1, padding=kSize // 2, bias=False, norm=norm,
                   act=act, num_groups=(dimList[3] // 4) // 16),
            myConv(dimList[3] // 8, dimList[3] // 16, kSize, stride=1, padding=kSize // 2, bias=False, norm=norm,
                   act=act, num_groups=(dimList[3] // 8) // 16),
            myConv(dimList[3] // 16, dimList[3] // 32, kSize, stride=1, padding=kSize // 2, bias=False, norm=norm,
                   act=act, num_groups=(dimList[3] // 16) // 16),
            myConv(dimList[3] // 32, 1, kSize, stride=1, padding=kSize // 2, bias=False, norm=norm, act=act,
                   num_groups=(dimList[3] // 32) // 16)
        )
        # Pyramid Level 4 (Simplified)
        self.decoder2_up1 = upConvLayer(dimList[3], dimList[3] // 2, 2, norm, act, (dimList[3] // 2) // 16)
        self.decoder2_reduc1 = myConv(dimList[3]//2+dimList[2], dimList[3] // 4 - 4, kSize=1, stride=1, padding=0,
        ...
        # Pyramid Level 3 (Simplified)
        self.decoder2_1_up2 = upConvLayer(dimList[3] // 4, dimList[3] // 4, 2, norm, act, (dimList[3] // 4) // 16)
        self.decoder2_1_reduc2 = myConv(dimList[3] // 4 + dimList[1], dimList[3] // 8 - 4, kSize=1, stride=1, padding=0,
        ...
        # Pyramid Level 2 and 1 remain unchanged
        ...
        self.decoder2_1_1_2 = myConv(dimList[3] // 16, 1, kSize, stride=1, padding=kSize // 2, bias=False, norm=norm,
                                     act=act, num_groups=(dimList[3] // 16) // 16)
        ...
        self.decoder2_1_1_1_2 = myConv(dimList[3] // 16, dimList[3] // 32, kSize, stride=1, padding=kSize // 2,
                                       bias=False, norm=norm, act=act, num_groups=(dimList[3] // 16) // 16)
        self.decoder2_1_1_1_3 = myConv(dimList[3] // 32, 1, kSize, stride=1, padding=kSize // 2, bias=False, norm=norm,
                                       act=act, num_groups=(dimList[3] // 32) // 16)
    ...
    def forward(self, x, rgb):
        ...
        # sigmoid函数把矩阵范围限制在0-1之间。
        final_depth = torch.sigmoid(final_depth)
        # decoder做完了，对应后面的d_res_list,depth.
...
class LDRN(nn.Module):
    ...
    def forward(self, x):
        # 返回的是多层特征图，经过backbone
        out_featList = self.encoder(x)
        # 输入是x，先对数据做一个encoder操作，x是经过backbone不同层后得到的不同特征图，先对x进行下采样，里面有两个参数，factor是倍率
        rgb_down2 = F.interpolate(x, scale_factor=0.5, mode='bilinear')  # 对x先进行一个下采样，0.5是采样倍率。
        ...
        # 深度估计一般对边界特征提取效果差，通过相减突出边界特征，差异就是边界,下面对应的是论文图上的L1,L2,L3,L4不同特征图上得到的差异所以大小不同。
        lap1 = x - rgb_up
